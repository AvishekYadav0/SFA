...
exports.createStaff = async (req, res) => {
  try {
    const { name, email, password, phone, province, designation } = req.body;
    ...
    const user = await User.create({
      name, email, password, phone, province: province || null,
      designation: designation || 'Marketing Staff',
      role: 'staff', isActive: true, createdBy: req.user._id,
    });
    res.status(201).json({
      success: true,
...
exports.updateUser = async (req, res) => {
  try {
    const { name, email, phone, province, designation } = req.body;
    const user = await User.findByIdAndUpdate(
      req.params.id,
      { name, email, phone, province: province || null, designation },
      { new: true }
    ).select('-password');
    if (!user) return res.status(404).json({ success: false, message: 'User not found' });
    res.json({ success: true, data: user });
